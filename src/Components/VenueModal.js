...
var props = {
  ...
  const ShowModal = (p) => {    
    ...
    async function deleteVenue(p) {
        const confirmed = window.confirm(`Are you sure you want to delete ${p.venueName}?`);
        if (confirmed) { 
            try {
                console.log(p._id);
                const response = await fetch(server_URL+'/owners/deleteVenues', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({id: p._id, owneremail: p.venueOwnerEmail})
                }).then(async (response)=>{
                    const jsonData = await response.json()
                    console.log(jsonData);
    ...
    function editVenue(p){
        ...
        document.getElementById('venueName').value = props.venueName;
        document.getElementById('location').value = props.location;
        document.getElementById('sportsName').value = props.sportName;
    ...
    async function updateVenue(event){
        ...
        const response = await fetch(server_URL+'/owners/updateVenues', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    if(p.venueName){        
        ...
        return (
            ...
            <div className="modal-overlay">
                <div id="venueModal" className="modal" style={{display : 'block'}}>                
                    ...
                    <div className={styles.card} style={{marginLeft: '0%', marginRight: '0%', marginTop: '0%'}}>
                        ...
                        <ul style={{marginLeft: '1%', marginRight: '1%'}}>
                            <li className={styles.listItem}><strong>Location:</strong> {props.location}</li>
                            <li className={styles.listItem}><strong>Description:</strong></li>
                            <li className={styles.listItem}>{props.description}</li> 
                            <li className={styles.listItem}><strong>Sports:</strong> {props.sportName}</li>
                ...
                {/* Edit modal */}
                <div id="editVenueModal" className="modal" style={{display : 'none'}}>                
                    ...
                    <div className={styles.card} style={{marginLeft: '0%', marginRight: '0%', marginTop: '0%'}}>
                        ...
                        <form className="form-inline" id="venue_form" >
                            <div className="row">
                                <div className={styles.imageContainer}>
                                    <button className={styles.closeBtn} onClick={() => {document.getElementById('editVenueModal').style.display = 'none'}}>
                                        X
                                    </button>
                            ...
                            <div className="row">
                                <center>
                                    <button className="btn btn-primary" style={{backgroundColor: '#08102a', borderColor: '#08102a', cursor: 'pointer', width: '20%', margin:'5%'}} onClick={updateVenue}>
                                        Update
                                    </button>
                                    <button className="btn btn-primary" style={{backgroundColor: '#08102a', borderColor: '#08102a', cursor: 'pointer', width: '15%', margin:'5%'}} onClick={() => {document.getElementById('editVenueModal').style.display = 'none'}} >
                                        Close
                                    </button>
                                </center>
                            </div>
